    ...
    for i in range(0, len(info_daily), num_companies):
        ...
        if daily[0][1] >= start_year and daily[0][2] >= start_day:
            ...
            if previous_stock_prices is not None:
                ...
                for c in range(num_companies):
                    increased.append(
                        current_stock_prices[c] > previous_stock_prices[c])
                y = pd.predict(market_segments, quarterly,
                               daily, current_stock_prices)
                print(market_segments, quarterly, daily, current_stock_prices)
                print("Predictions (year, day):",
                      daily[0][1], daily[0][2], y, "Target:", increased)
                for c in range(num_companies):
                    if y[c] == increased[c]:
                        correct += 1
                    total += 1
